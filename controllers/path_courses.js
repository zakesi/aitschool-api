...
const PathCoursesControllers = {
  sort: async function(req, res, next) {
    try {
      ...
      await Promise.all(
        data.map(async item => {
          let zhi = await PathCourses.where({
            path_id: item.path_id,
            course_id: item.course_id
          }).update({ sort: Number(item.index) });
          return zhi;
        })
      );
  ...
  store: async function(req, res, next) {
    try {
      ...
      if (gist.length != data.length)
        return res.json({ error_code: 1, message: "请勿填写无效课程" });
      await Promise.all(
        data.map(async item => {
